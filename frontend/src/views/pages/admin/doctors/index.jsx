...
const AdminDoctors = () => {
  ...
  const showModal = (type, doctor = null) => {
    ...
    if (type === "edit" || type === "view") {
      form.setFieldsValue({
        ...
        base_time: doctor.base_time ? dayjs(doctor.base_time, "HH:mm") : null,
        room: doctor.room,
      });
    } else {
      form.resetFields();
  ...
  const handleSubmit = async () => {
    ...
    if (modalType === "add") {
      ...
      if (res && res.data) {
        setDoctorsData((prev) =>
          prev.map((item) =>
            item.account._id === selectedDoctor.account._id ? res.data : item
          )
        );
        setIsModalVisible(false);
        form.resetFields();
  ...
  const handleDelete = async (id) => {
    ...
    setDoctorsData((prev) => prev.filter((item) => item.account._id !== id));
  ...
  const columns = [
    ...
    },
    {
      ...
      filters: Array.from(
        new Set(doctorsData.map((doctor) => doctor?.specialization?.name))
      )
    ...
    <div className="doctors-page">
      ...
      {searchText && (
        <div style={{ marginBottom: 12 }}>
          <span>
            Kết quả tìm kiếm cho: <b>{searchText}</b> ({filteredDoctors.length}{" "}
            kết quả)
          </span>
        </div>
      )}
        ...
        <Form form={form} layout="vertical" disabled={modalType === "view"}>
          {modalType !== "add" && selectedDoctor && (
            <div className="avatar-preview">
              <Image
                src={selectedDoctor.account.avatar || "/placeholder.svg"}
                alt="Avatar"
                width={100}
                height={100}
          ...
          <div className="form-section">
            ...
            <div className="form-row">
              <Form.Item
                ...
                rules={[
                  { required: true, message: "Vui lòng nhập thời gian khám" },
                ]}
                className="form-col"
              >
          ...
          {modalType === "view" && selectedDoctor && (
            <div className="form-section">
              <h3 className="section-title">Thông tin chuyên khoa</h3>
              <div className="specialization-info">
                <div className="specialization-image">
                  <Image
                    src={
                      selectedDoctor.specialization.image || "/placeholder.svg"
                    }
                    alt="Specialization"
                    width={100}
                    height={100}
                ...
                <div className="specialization-details">
                  <p>
                    <strong>Mô tả:</strong>{" "}
                    {selectedDoctor.specialization.description}
                  </p>
                  <p>
